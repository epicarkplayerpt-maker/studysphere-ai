...
async function executeSearch(query) {
    ...
    try {
        ...
        logger_1.default.warn('DuckDuckGo request failed: %s', ddgErr.message || ddgErr);
    }
    // 2. If DuckDuckGo returned 0 results or was blocked (202), query Bing Search
    if (results.length === 0) {
        try {
            logger_1.default.info('DuckDuckGo search returned 0 results. Falling back to Bing Search...');
            ...
            if (response.ok) {
                ...
                logger_1.default.info('Bing Search parsed %d results as fallback.', results.length);
            }
            else {
                logger_1.default.warn('Bing Search failed with status code %d', response.status);
            }
        }
        catch (bingErr) {
            logger_1.default.warn('Bing Search request failed: %s', bingErr.message || bingErr);
        }
    }
    // 3. Fallback to Yahoo Search if both DDG and Bing return 0 results
    if (results.length === 0) {
        try {
            logger_1.default.info('Bing Search returned 0 results. Falling back to Yahoo Search...');
            ...
            if (response.ok) {
                ...
                logger_1.default.info('Yahoo Search parsed %d results as secondary fallback.', results.length);
            }
            else {
                logger_1.default.warn('Yahoo Search failed with status code %d', response.status);
            }
        }
        catch (yahooErr) {
            logger_1.default.warn('Yahoo Search request failed: %s', yahooErr.message || yahooErr);
        }
    }
    return results;
}
...
async function fetchPageContent(url) {
    try {
        ...
        logger_1.default.warn('Failed to fetch page content for URL "%s": %s', url, error.message || error);
        return '';
    }
}
