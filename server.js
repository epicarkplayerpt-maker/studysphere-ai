...
const allowedOrigins = process.env.ALLOWED_ORIGINS ? process.env.ALLOWED_ORIGINS.split(',') : ['http://localhost:3000', 'http://localhost:5173', 'http://127.0.0.1:5500'];
app.use(cors({ origin: (origin, callback) => (!origin || allowedOrigins.includes(origin)) ? callback(null, true) : callback(new Error('CORS blocked')), credentials: true }));
...
const apiLimiter = rateLimit({
    ...
    validate: { xForwardedForHeader: false, ip: false }, // FIX: Silences ERR_ERL_KEY_GEN_IPV6 locally
